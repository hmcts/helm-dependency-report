...
# prs=$(gh search prs "Update Helm release" --owner hmcts --author app/renovate --state=open --created="*..$(date -v-7d  +"%Y"-"%m"-"%d")" --sort=created --json repository | jq -r '. | unique_by(.repostory.name)' | jq -r '.[].repository.name')

repos=$(gh search prs "Update Helm release" --repo hmcts/sds-toffee-frontend  --author app/renovate --state=open --created="*..$(date -v-7d  +"%Y"-"%m"-"%d")" --sort=created --json repository | jq -r '. | unique_by(.repostory.name)' | jq -r '.[].repository.name')
